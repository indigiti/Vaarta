...
if ( ! class_exists( 'CSCO_Theme_Demos' ) ) {
	...
	class CSCO_Theme_Demos {
		...
		public function add_menu_page() {
			add_submenu_page( 'themes.php', esc_html__( 'Theme Demos', 'caards' ), esc_html__( 'Theme Demos', 'caards' ), 'manage_options', $this->menu_slug, array( $this, 'html_carcase' ), 2 );
		}
		...
		public function set_demos( $demos ) {
			$this->demos = apply_filters( csco_get_license_data( 'demos_list' ), $this->demos );
		}
		...
		public function html_import_data() {
			...
			if ( $demo_id ) {
				...
				if ( ! isset( $this->demos[ $demo_id ] ) ) {
					wp_send_json_error( esc_html__( 'Invalid demo content id.', 'caards' ) );
					wp_die();
				}
				...
				<div class="cs-import-data">
					<?php if ( $demo_plugins ) { ?>
						<div class="cs-import-plugins">
							<div class="cs-import-subheader">
								<?php esc_html_e( 'Install Plugins', 'caards' ); ?>
							</div>
							...
							foreach ( $demo_plugins as $plugin ) {
								...
								<form>
									<div class="cs-switcher">
										...
										<div class="cs-tooltip"><?php esc_html_e( 'Required plugin will be installed', 'caards' ); ?></div>
									</div>
									...
									<input type="hidden" name="step_name" value="<?php esc_attr( esc_html__( 'Installing and activating', 'caards' ) ); ?> <?php echo esc_attr( $plugin['name'] ); ?>...">
					...
					<div class="cs-import-content">
						<div class="cs-import-subheader">
							<?php esc_html_e( 'Import Content', 'caards' ); ?>
						</div>
								...
								<?php foreach ( $kits as $kit ) { ?>
									<form>
										<div class="cs-switcher">
											...
											<input type="hidden" name="step_name" value="<?php esc_html_e( 'Importing', 'caards' ); ?> <?php echo esc_attr( $kit['label'] ); ?> ...">
						...
						<?php if ( isset( $demo_data['import']['customizer'] ) && $demo_data['import']['customizer'] ) { ?>
							<form>
								<div class="cs-switcher">
									<?php esc_html_e( 'Customizer', 'caards' ); ?> <input class="cs-checkbox" type="checkbox" name="url" value="<?php echo esc_attr( $demo_data['import']['customizer'] ); ?>" checked>

									<div class="cs-switch"><span class="cs-switch-slider"></span></div>
								</div>

								<input type="hidden" name="step_name" value="<?php esc_html_e( 'Importing customizer options...', 'caards' ); ?>">
						...
						<?php if ( isset( $demo_data['import']['options'] ) && $demo_data['import']['options'] ) { ?>
							<form>
								<div class="cs-switcher">
									<?php esc_html_e( 'Options', 'caards' ); ?> <input class="cs-checkbox" type="checkbox" name="url" value="<?php echo esc_attr( $demo_data['import']['options'] ); ?>" checked>

									<div class="cs-switch"><span class="cs-switch-slider"></span></div>
								</div>

								<input type="hidden" name="step_name" value="<?php esc_html_e( 'Importing options...', 'caards' ); ?>">
						...
						<?php if ( isset( $demo_data['import']['widgets'] ) && $demo_data['import']['widgets'] ) { ?>
							<form>
								<div class="cs-switcher">
									<?php esc_html_e( 'Widgets', 'caards' ); ?> <input class="cs-checkbox" type="checkbox" name="url" value="<?php echo esc_attr( $demo_data['import']['widgets'] ); ?>" checked>

									<div class="cs-switch"><span class="cs-switch-slider"></span></div>
								</div>

								<input type="hidden" name="step_name" value="<?php esc_html_e( 'Importing widgets...', 'caards' ); ?>">
						...
						<form class="hidden">
							<div class="cs-switcher">
								<?php esc_html_e( 'Finish', 'caards' ); ?> <input class="cs-checkbox" type="checkbox" name="finish" value="1" checked>

								<div class="cs-switch"><span class="cs-switch-slider"></span></div>
							</div>

							<input type="hidden" name="step_name" value="<?php esc_html_e( 'Finishing setup...', 'caards' ); ?>">
				...
				wp_send_json_error( esc_html__( 'Demo content id not set.', 'caards' ) );
			}
		...
		public function html_carcase() {
			...
			<div class="cs-wrap cs-demos-page">
				...
				<div class="wrap">
					<h1 class="hidden"><?php esc_html_e( 'Theme Demos', 'caards' ); ?></h1>
										...
										<div class="cs-demo-thumbnail">
											...
											<?php if ( isset( $demo['preview'] ) && $demo['preview'] ) { ?>
												<div class="cs-demo-preview">
													<span>
														<?php esc_html_e( 'Preview Demo', 'caards' ); ?>
													</span>
												</div>
											<?php } ?>
										</div>
						...
						<div class="cs-import-container">
							<div class="cs-import-step cs-import-step-active cs-import-start">
								<div class="cs-import-header">
									<?php esc_html_e( 'Import Theme', 'caards' ); ?>
								</div>
							...
							<div class="cs-import-step cs-import-process">
								<div class="cs-import-header">
									<?php esc_html_e( 'Installing', 'caards' ); ?>
								</div>

								<div class="cs-import-output">
									<div class="cs-import-desc">
										<?php esc_html_e( 'Please be patient and don\'t refresh this page, the import process may take a while, this also depends on your server.', 'caards' ); ?>
									</div>
							...
							<div class="cs-import-step cs-import-finish">
								<div class="cs-import-info">
									...
									<div class="cs-import-title">
										<?php esc_html_e( 'Imported Succefully', 'caards' ); ?>
									</div>

									<div class="cs-import-desc">
										<?php esc_html_e( 'Go ahead, customize the text, images and design to make it yours!', 'caards' ); ?>
									</div>

									<div class="cs-import-customize">
										<a href="<?php echo esc_url( admin_url( '/customize.php' ) ); ?>" class="button button-primary" target="_blank">
											<?php esc_html_e( 'Customize', 'caards' ); ?>
										</a>
									</div>
								</div>
								...
								<div class="cs-import-actions">
									<a href="<?php echo esc_url( add_query_arg( 'page', $this->dashboard_menu_slug, admin_url( 'themes.php' ) ) ); ?>" class="cs-link">
										<?php esc_html_e( 'Return to Dashboard', 'caards' ); ?>
									</a>

									<a href="<?php echo esc_url( home_url() ); ?>" class="cs-button button" target="_blank">
										<?php esc_html_e( 'View Site', 'caards' ); ?>
									</a>
								</div>
							</div>
						</div>
				...
				<div class="cs-preview">
					...
					<iframe id="cs-preview-iframe" class="cs-preview-iframe"></iframe>
				</div>
			</div>
			<?php
		}
		...
		public function admin_enqueue_scripts( $page ) {
			wp_enqueue_script( 'csco-theme-demos', get_theme_file_uri( '/core/theme-demos/assets/theme-demos.js' ), array( 'jquery' ), filemtime( get_theme_file_path( '/core/theme-demos/assets/theme-demos.js' ) ), true );

			wp_localize_script( 'csco-theme-demos', 'cscoThemeDemosConfig', array(
				'ajax_url'       => admin_url( 'admin-ajax.php' ),
				'nonce'          => wp_create_nonce( 'nonce' ),
				'failed_message' => esc_html__( 'Something went wrong, contact support.', 'caards' ),
			) );

			// Styles.
			wp_enqueue_style( 'csco-theme-demos', get_theme_file_uri( '/core/theme-demos/assets/theme-demos.css' ), array(), filemtime( get_theme_file_path( '/core/theme-demos/assets/theme-demos.css' ) ) );

			// Add RTL support.
			wp_style_add_data( 'caards', 'rtl', 'replace' );
		}
	}

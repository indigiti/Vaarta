...
class CSCO_Manager_Import {
	...
	public function ajax_import_plugin() {
		...
		if ( ! isset( $_POST['plugin_slug'] ) || ! sanitize_text_field( $_POST['plugin_slug'] ) ) {
			$this->send_json_error( esc_html__( 'Unknown slug in a plugin.', 'caards' ) );
		}

		if ( ! isset( $_POST['plugin_path'] ) || ! sanitize_text_field( $_POST['plugin_path'] ) ) {
			$this->send_json_error( esc_html__( 'Unknown path in a plugin.', 'caards' ) );
		}
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			$this->send_json_error( esc_html__( 'Insufficient permissions to install the plugin.', 'caards' ) );
		}
		...
		$this->send_json_error( esc_html__( 'Failed to initialize or activate importer plugin.', 'caards' ) );
	...
	public function ajax_import_contents() {
		...
		if ( ! isset( $_POST['url'] ) || ! sanitize_text_field( $_POST['url'] ) ) {
			$this->send_json_error( esc_html__( 'The url address of the demo content is not specified.', 'caards' ) );
		}
		...
		if ( ! $xml_file_path ) {
			...
			if ( ! current_user_can( 'edit_theme_options' ) ) {
				$this->send_json_error( esc_html__( 'You are not permitted to import contents.', 'caards' ) );
			}

			if ( ! isset( $file_url ) ) {
				$this->send_json_error( esc_html__( 'No XML file specified.', 'caards' ) );
			}
	...
	public function ajax_import_customizer() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			$this->send_json_error( esc_html__( 'You are not permitted to import customizer.', 'caards' ) );
		}

		if ( ! isset( $file_url ) ) {
			$this->send_json_error( esc_html__( 'No customizer JSON file specified.', 'caards' ) );
		}
	...
	public function ajax_import_widgets() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			$this->send_json_error( esc_html__( 'You are not permitted to import widgets.', 'caards' ) );
		}

		if ( ! isset( $file_url ) ) {
			$this->send_json_error( esc_html__( 'No widgets WIE file specified.', 'caards' ) );
		}
	...
	public function ajax_import_options() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			$this->send_json_error( esc_html__( 'You are not permitted to import options.', 'caards' ) );
		}

		if ( ! isset( $file_url ) ) {
			$this->send_json_error( esc_html__( 'No options JSON file specified.', 'caards' ) );
		}

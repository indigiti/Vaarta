...
if ( ! class_exists( 'CSCO_Customizer_Output_Styles' ) ) {
	...
	class CSCO_Customizer_Output_Styles {
			...
			foreach ( $fields as $field ) {
				...
				foreach ( $field['output'] as $output ) {
					...
					if ( ! is_array( $value ) ) {
						...
						if ( isset( $output['reverse'] ) && isset( $output['reverse_max'] ) ) {
							$multi_selector = __return_empty_string();

							for ( $iteration = 1; $iteration <= $output['reverse_max']; $iteration++ ) {
								if ( $iteration <= $value ) {
									continue;
								}
								$multi_selector .= ( $multi_selector ? ', ' : '' ) . str_replace( '$numb', $iteration, $output['reverse'] );
							}

							$output['element'] = $multi_selector;
						}
		...
		public function enqueue_output_styles( $page ) {
			...
			wp_register_style( 'csco-customizer-output-styles', false, array(), csco_get_theme_data( 'Version' ) );

			wp_enqueue_style( 'csco-customizer-output-styles' );

			wp_add_inline_style( 'csco-customizer-output-styles', $this->get_output_styles() );
		}
	}

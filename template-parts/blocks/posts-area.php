...
if ( $posts->have_posts() ) {
	...
	// Min Height.
	$min_height = isset( $options['card_min_height'] ) ? trim( $options['card_min_height'] ) : null;

	// Posts Layout.
	$main_classes = 'cs-block-posts-layout-' . esc_attr( $attributes['layout'] );

	// Tile hover.
	if ( 'tile-type-2' === $attributes['layout'] || 'tile-type-4' === $attributes['layout'] ) {
		$main_classes .= ' cs-block-posts-layout-tile-hover';
	}

	// Top post meta.
	if ( isset( $options['top_meta'] ) && 'count' === $options['top_meta'] ) {
		$main_classes .= ' cs-block-numbered';
	}
			...
			if ( 'standard' === $pagination_type ) {
				...
				if ( $posts->max_num_pages > 1 ) {
					...
					if ( $base_url ) {
						...
						cnvs_alert_warning( esc_html__( 'Please select a page for your blog posts in Settings &rarr; Reading to display pagination.', 'caards' ) );
					}

...
<div class="cs-page__header <?php echo esc_attr( $class ); ?>">
		...
		if ( is_author() ) {
			...
			$subtitle  = esc_html__( 'All Posts By', 'caards' );
			...
			if ( is_category() ) {
				$subtitle = get_theme_mod( 'category_subtitle', esc_html__( 'Browsing Category', 'caards' ) );
			} elseif ( is_tag() ) {
				$subtitle = esc_html__( 'Browsing Tag', 'caards' );
			} else {
				$subtitle = '';
			}
			...
			?>
			<span class="cs-page__subtitle"><?php esc_html_e( 'Search Results', 'caards' ); ?></span>
			...
			?>
			<h1 class="cs-page__title"><?php esc_html_e( '404', 'caards' ); ?></h1>
			<?php
		}

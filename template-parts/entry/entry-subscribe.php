...
if ( shortcode_exists( 'powerkit_subscription_form' ) && $subscription_form ) {
	$subscribe_title = get_theme_mod( 'post_subscribe_title', esc_html__( 'Subscribe to our newsletter', 'caards' ) );
	$subscribe_text  = get_theme_mod( 'post_subscribe_text', esc_html__( 'Get notified of the best deals on our WordPress themes', 'caards' ) );
					...
					<?php if ( $subscribe_title || $subscribe_text ) { ?>
						<div class="cs-site-subscribe__info">
							<?php if ( $subscribe_title ) { ?>
								<h5 class="cs-site-subscribe__title"><?php echo wp_kses( $subscribe_title, 'csco' ); ?></h5>
							<?php } ?>
